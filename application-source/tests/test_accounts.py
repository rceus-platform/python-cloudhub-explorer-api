...
def test_add_account_manual(authenticated_client, db, test_user):
    ...
    response = authenticated_client.post(
        "/accounts/add",
        json={"provider": "gdrive", "access_token": "test_token"}
    )
    assert response.status_code == 200
...
@patch("app.api.routes.accounts.Flow")
def test_google_callback(mock_flow, authenticated_client, db, test_user):
...
def test_mega_login_success(mock_get_mega, authenticated_client, db, test_user):
